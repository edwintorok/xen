...
let do_argv =
  ...
  let speclist =
    [ ("--no-domain-init", Arg.Unit (fun () -> domain_init := false),
      ...
      ("--config-validate", Arg.Set config_validate, "Test validity of config file");
  ...
  let () =
    if !help then begin
      if !live_reload then
        (*
          Transform --live --help into --config-validate for backward compat with
