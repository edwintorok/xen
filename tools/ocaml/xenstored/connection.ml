...
module BoundedPipe : sig
	...
	(** [create ~capacity ~destination] creates a bounded pipe with a local buffer
			holding at most [capacity] items.
			Once the buffer is full it will not accept further items.
			Items from the pipe are flushed into [destination] as long as it accepts items.
			The destination could be another pipe.
	 *)
...
let create xbcon dom =
	...
	(* the actual capacity will be lower, this is used as an overflow buffer:
		anything that doesn't fit elsewhere gets put here, only limited by the amount
		of watches that you can generate with a single xenstore command
		(which is finite, although possibly very large in theory for Dom0).
		Once the pipe here has any contents the domain is blocked from sending more commands
		until it is empty again though.
	*)
