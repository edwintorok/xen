...
(** [recurse_filter_map f tree] applies [f] on each node in the tree recursively,
    ...
    Note that the nodes removed this way won't generate watchevents
*)
let recurse_filter_map f =
  let invalid = -1 in
  let is_valid _ node = node.perms.owner <> invalid in
	let rec walk node =
	  (* Map.filter_map is 4.11+ only *)
	  let node = { node with
	    children = SymbolMap.map walk node.children |> SymbolMap.filter is_valid
	  } in
	  match f node with
	  | Some keep -> keep
