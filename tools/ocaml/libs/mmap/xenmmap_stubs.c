...
static int mmap_interface_init(struct mmap_interface *intf, int fd, int pflag,
                               int mflag, int len, int offset)
{
    intf->len = len;
...
CAMLprim value stub_mmap_init(value fd, value pflag, value mflag, value len,
                              value offset)
{
    ...
    if ( mmap_interface_init(Intf_val(result), Int_val(fd), c_pflag, c_mflag,
...
CAMLprim value stub_mmap_write(value intf, value data, value start, value len)
{
