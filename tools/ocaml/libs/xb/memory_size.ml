...
let make size = { size; parent = Immutable; item_overhead = unboxed }
...
let add a b =
  ...
  | _ ->
      let t = { size; parent = UpdatableUnset; item_overhead = unboxed } in
...
type ('a, 'b) fields = 'b t
let record_start _t = { size = boxed; parent = UpdatableUnset; item_overhead = unboxed }
...
let container_create ~initial ~item_overhead =
  let t = { size = initial.size; parent = UpdatableUnset; item_overhead = item_overhead.size } in
  set_parent initial ~parent:t;
  t
