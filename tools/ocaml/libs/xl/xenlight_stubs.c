...
void ctx_finalize(value ctx) { libxl_ctx_free(CTX); }

static struct custom_operations libxl_ctx_custom_operations = {
    "libxl_ctx_custom_operations", ctx_finalize /* custom_finalize_default */,
...
CAMLprim value stub_libxl_ctx_alloc(value logger)
{
    ...
    ret = libxl_ctx_alloc(&ctx, LIBXL_VERSION, 0,
                          (xentoollog_logger *)Xtl_val(logger));
    if ( ret != 0 )
        failwith_xl(ERROR_FAIL, "cannot init context");

    handle
        = caml_alloc_custom(&libxl_ctx_custom_operations, sizeof(ctx), 0, 1);
    Ctx_val(handle) = ctx;
...
static int libxl_key_value_list_val(libxl_key_value_list *c_val, value v)
{
    CAMLparam1(v);
    CAMLlocal1(elem);
    int nr, i;
...
static value Val_key_value_list(libxl_key_value_list *c_val)
{
    ...
    list = Val_emptylist;
    for ( i = libxl_string_list_length((libxl_string_list *)c_val) - 1; i >= 0;
          i -= 2 )
    {
...
static libxl_asyncop_how *aohow_val(value async)
{
    ...
    if ( async != Val_none )
    {
         ...
         * registered with the OCaml GC. This ensures that the GC does not
         * remove the corresponding OCaml heap blocks, and allows the GC to
         * update the value when blocks are moved around, while libxl is free
         * to copy the pointer if it needs to. The for_callback pointer must
         * always be non-NULL. */
...
value stub_libxl_domain_create_restore(value ctx, value domain_config,
                                       value params, value async, value unit)
{
...
value stub_libxl_domain_shutdown(value ctx, value domid, value async,
                                 value unit)
{
...
#define Console_reader_val(x)                                                 \
    (*((libxl_xen_console_reader **)Data_custom_val(x)))
...
value stub_libxl_xen_console_read_line(value ctx, value reader)
{
    ...
    char *c_line;
    libxl_xen_console_reader *cr
        = (libxl_xen_console_reader *)Console_reader_val(reader);
...
value Val_poll(short event)
{
    ...
    case POLLNVAL:
        ...
        break;
    default:
        failwith_xl(ERROR_FAIL, "cannot convert poll event value");
        break;
    }
...
void disaster(void *user, libxl_event_type type, const char *msg, int errnoval)
{
    caml_leave_blocking_section();
    CAMLparam0();
    CAMLlocalN(args, 4);
