...
module Tree = struct
  ...
  let rec add t path value =
    ...
    | hd :: tl ->
        ...
        let children = M.update key (fun old ->
  ...
  let rec remove t = function
    ...
    | hd :: tl ->
        let key = Symbol.of_string hd in
        let children = M.update key (function
          | None -> None
          | Some old -> remove old tl) t.children
        in
        Some { t with children }

  let remove t path =
    Option.value ~default:empty (remove t path)
end
...
let () =
    let () = Random.init 42 in (* deterministic *)
